...
export async function processIssue(
  ...
  const routing = selectModel({ stack: parsed.stackTrace });
  logger.setContext({ tier: routing.tier });

  const execution = executeTier
    ? await executeTier(parsed, routing, { costTracker })
    : await runMockExecution(parsed, routing, costTracker, mockAI ? issueNumber : parsed.issueNumber ?? issueNumber);
  ...
  metrics.recordFix(routing.tier, testsPassed, cost);
  ...
  const prInput = {
    ...
    body: `Fixes #${issueNumber}\n\nAutomated fix generated by Homeostat (Tier ${routing.tier}).`,
    base: 'main',
